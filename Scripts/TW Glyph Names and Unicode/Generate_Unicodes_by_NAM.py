...
class fontGlyphNameUnicodes:
    ...
    def _parseNamFile(self, namFile):
        reNAM = re.compile(
            r"^0x(?P<UNI>[A-Fa-f0-9]{4,5})\s+(?P<type>[>!]*)(?P<name>[A-Za-z0-9_\.-]+)"
        )
        n2u = {}
        ...
        for g in f.glyphs:
            ...
            if mod:
                ...
                print(
                    "{}: [{}] -> [{}]".format(
                        n,
                        " ".join(["%04X" % u for u in oldus]),
                        " ".join(["%04X" % u for u in g.unicodes]),
                    )
                )
                if flagModified:
                    lg = fl6.flGlyph(g, f)
...
class dlg_generateUnicode(QtGui.QDialog):
    def __init__(self):
        ...
        for i in range(len(colorNames)):
            ...
            self.cmb_flag.setItemData(
                i, QtGui.QColor(colorNames[i]), QtCore.Qt.DecorationRole
            )
    ...
    def openNamFile(self):
        namPath = QtGui.QFileDialog.getOpenFileName(
            None, "Choose NAM file", self.namFolder, "Unicode-to-glyphname NAM (*.nam)"
        )
        if namPath:
            self.fgnu.readFromNamPath(namPath)
            print(
                'Opened {} with Unicode mappings for {} glyphs.\nClick "Generate Unicodes" to generate the Unicodes based on these mappings.'.format(
                    namPath, len(self.fgnu.n2u)
                )
            )
